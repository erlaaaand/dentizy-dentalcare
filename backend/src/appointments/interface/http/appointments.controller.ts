...
import { AppointmentResponseDto, PaginatedAppointmentResponseDto } from '../../applications/dto/appointment-response.dto';
...
export class AppointmentsController {
    ...
    @ApiOperation({
        ...
        description: 'Membuat janji temu baru dengan validasi waktu dan conflict detection',
    })
    @ApiResponse({
        status: 201,
    ...
    @ApiResponse({ status: 404, description: 'Pasien atau dokter tidak ditemukan' })
    @ApiResponse({ status: 409, description: 'Konflik jadwal dengan appointment lain' })
    async create(
    ...
    @ApiResponse({ status: 409, description: 'Status tidak valid untuk diselesaikan' })
    async complete(
    ...
    @ApiOperation({
        ...
        description: 'Membatalkan appointment dengan validasi business rules (< 24 jam hanya Kepala Klinik)',
    })
    @ApiParam({ name: 'id', description: 'ID Appointment' })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 404, description: 'Appointment tidak ditemukan' })
    @ApiResponse({ status: 403, description: 'Tidak memiliki akses atau pembatalan < 24 jam' })
    @ApiResponse({ status: 409, description: 'Status tidak valid untuk dibatalkan' })
    async cancel(
        @Param('id', ParseIntPipe) id: number,
        @GetUser() user: User
    ): Promise<AppointmentResponseDto> {
    ...
    @ApiQuery({ name: 'status', required: false, enum: ['dijadwalkan', 'selesai', 'dibatalkan'] })
    ...
    @ApiResponse({ status: 404, description: 'Appointment tidak ditemukan' })
    @ApiResponse({ status: 409, description: 'Tidak bisa dihapus karena sudah ada medical record' })
    async remove(
        @Param('id', ParseIntPipe) id: number,
        @GetUser() user: User
