...
export class AppointmentEventListener {
    ...
    async handleAppointmentCreated(event: AppointmentCreatedEvent) {
        ...
        if (event.shouldScheduleReminder) {
            try {
                await this.notificationsService.scheduleAppointmentReminder(event.appointment);
                this.logger.log(`📧 Reminder scheduled for appointment #${event.appointment.id}`);
            } catch (error) {
                this.logger.error(`❌ Failed to schedule reminder: ${error.message}`, error.stack);
            }
        }
    }
    ...
    async handleAppointmentCancelled(event: AppointmentCancelledEvent) {
        ...
        try {
            ...
            this.logger.log(`📧 Reminders cancelled for appointment #${event.appointment.id}`);
        } catch (error) {
            this.logger.error(`❌ Failed to cancel reminders: ${error.message}`, error.stack);
        }
    }
        ...
        if (event.isTimeUpdated) {
            try {
                ...
                if (
                    ...
                    await this.notificationsService.scheduleAppointmentReminder(event.appointment);
                    this.logger.log(`📧 Reminder rescheduled for appointment #${event.appointment.id}`);
                }
            } catch (error) {
                this.logger.error(`❌ Failed to reschedule reminder: ${error.message}`, error.stack);
            }
        }
    }
    ...
    async handleAppointmentDeleted(event: AppointmentDeletedEvent) {
        this.logger.log(`🗑️ Appointment deleted: #${event.appointmentId} by user #${event.deletedBy}`);

        try {
            await this.notificationsService.cancelRemindersFor(event.appointmentId);
            this.logger.log(`📧 Reminders cancelled for deleted appointment #${event.appointmentId}`);
        } catch (error) {
            this.logger.error(`❌ Failed to cancel reminders: ${error.message}`, error.stack);
        }
    }
