    ...
    async execute(dto: PublicBookingDto): Promise<Appointment> {
        try {
            ...
            if (existingPatient) {
                ...
                const inputBirthDate = new Date(dto.tanggal_lahir).toISOString().split('T')[0];
                const dbBirthDate = new Date(existingPatient.tanggal_lahir!).toISOString().split('T')[0];
                ...
                const newPatient = await this.patientCreationService.execute(newPatientDto);
                ...
                this.logger.log(`🆕 New provisional patient created: #${patientId} (waiting verification)`);
            }
            ...
            const appointment = await this.appointmentCreationService.execute(appointmentDto);
