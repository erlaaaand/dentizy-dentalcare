...
export class AppointmentsService {
    ...
    async findAll(user: User, queryDto: FindAppointmentsQueryDto): Promise<PaginatedAppointmentResponseDto> {
        const result = await this.searchService.execute(user, queryDto);
        return this.mapper.toPaginatedResponse(result.data, result.count, result.page, result.limit);
    }
    ...
    async update(id: number, dto: UpdateAppointmentDto, user?: User): Promise<AppointmentResponseDto> {
        if (dto.status === AppointmentStatus.SELESAI && dto.medical_record && user) {
    ...
    private async handleCompletionWithMedicalRecord(
        ...
        try {
            ...
            if (!existingAppt) throw new NotFoundException('Appointment tidak ditemukan');
            ...
            if (treatmentIds.length > 0) {
                for (const treatmentId of treatmentIds) {
                    const treatmentMaster = await this.treatmentsService.findOne(treatmentId);
                    const hargaFix = treatmentMaster.harga ? Number(treatmentMaster.harga) : 0;
            ...
            throw new BadRequestException(`Gagal memproses: ${(error as any).message}`);
        } finally {
