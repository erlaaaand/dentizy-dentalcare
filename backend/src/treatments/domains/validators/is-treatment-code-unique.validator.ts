...
export class IsTreatmentCodeUniqueConstraint implements ValidatorConstraintInterface {
    ...
    async validate(kodePerawatan: string, args: ValidationArguments): Promise<boolean> {
        if (!kodePerawatan) return true;

        const excludeId = (args.object as any).id;
        return !(await this.treatmentRepository.isKodeExists(kodePerawatan, excludeId));
    }
