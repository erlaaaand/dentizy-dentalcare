...
export class TreatmentTransactionService {
    ...
    async withTransaction<T>(work: (queryRunner: QueryRunner) => Promise<T>): Promise<T> {
    ...
    async bulkUpdate(updates: { id: number; data: Partial<Treatment> }[]): Promise<void> {
    ...
    async moveToCategory(treatmentIds: number[], newCategoryId: number): Promise<number> {
        return await this.withTransaction(async (queryRunner) => {
