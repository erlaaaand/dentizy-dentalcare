...
export class FingerprintVerificationService {
    ...
    async execute(dto: VerifyFingerprintDto): Promise<VerifyFingerprintResponseDto> {
    ...
    private async verifyOneToMany(
        ...
        if (fingerprints.length === 0) {
            throw new NotFoundException('Tidak ada sidik jari terdaftar dalam sistem');
        }
    ...
    private async matchFingerprints(
        ...
        for (const fingerprint of fingerprints) {
            const score = await device.match(
                templateData,
    ...
    private async updateVerificationStats(fingerprint: Fingerprint): Promise<void> {
