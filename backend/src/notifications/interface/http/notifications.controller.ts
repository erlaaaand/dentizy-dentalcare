...
export class NotificationsController {
    ...
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Jumlah per halaman (default: 20)' })
    @ApiQuery({ name: 'status', required: false, enum: ['pending', 'sent', 'failed'] })
    @ApiQuery({ name: 'type', required: false, enum: ['email_reminder', 'sms_reminder', 'whatsapp_confirmation'] })
    @ApiResponse({
        status: 200,
    ...
    @ApiOperation({
        ...
        description: 'Mendapatkan status semua cron jobs notifikasi (Kepala Klinik only)'
    })
    @ApiResponse({
        status: 200,
    ...
    @ApiResponse({ status: 404, description: 'Notifikasi tidak ditemukan' })
    @ApiResponse({ status: 400, description: 'Notifikasi tidak dalam status FAILED atau sudah melebihi batas retry' })
    ...
    @ApiOperation({
        ...
        description: 'Mencoba ulang pengiriman semua notifikasi yang gagal (Kepala Klinik only)'
    })
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Maksimal 50' })
    @ApiResponse({
        status: 200,
    ...
    @ApiOperation({
        ...
        description: 'Memicu pemrosesan notifikasi secara manual (Kepala Klinik only)'
    })
    @ApiResponse({
        status: 200,
    ...
    @ApiOperation({
        ...
        description: 'Menghentikan semua cron jobs notifikasi (Kepala Klinik only) - USE WITH CAUTION'
    })
    @ApiResponse({
        status: 200,
