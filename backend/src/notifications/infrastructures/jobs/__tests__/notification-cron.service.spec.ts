...
describe('NotificationCronService', () => {
  ...
  beforeEach(async () => {
    ...
    (mockSchedulerRegistry.getCronJobs as jest.Mock).mockReturnValue(
      new Map([
        [
          'send-pending-notifications',
          {
            ...mockCronJob,
            nextDate: mockCronJob.nextDate,
            running: true,
  ...
  describe('handleSendPendingNotifications', () => {
    ...
    it('should skip execution if isProcessing is true (prevent concurrent execution)', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        '⏩ Skipping: Previous batch still processing',
      );
  ...
  describe('handleCleanupStale', () => {
    ...
    it('should reset stale PROCESSING notifications to PENDING', async () => {
      ...
      // Assert
      expect(
        mockNotificationRepository.findStaleProcessing,
      ).toHaveBeenCalledWith((service as any).STALE_CLEANUP_TIMEOUT);
    ...
    it('should log debug if no stale notifications are found', async () => {
      ...
      expect(mockLogger.debug).toHaveBeenCalledWith(
        `✅ [${jobName}] No stale notifications found`,
      );
    });
    ...
    it('should log error if repository throws an error', async () => {
      ...
      mockNotificationRepository.findStaleProcessing.mockRejectedValue(
        repoError,
      );
  ...
  describe('handleHealthCheck', () => {
    ...
    it('should log a warning if failed count is high (> 100)', async () => {
      ...
      mockNotificationRepository.getStatistics.mockResolvedValue(
        highFailedStats,
      );
  ...
  describe('Utility Methods', () => {
    ...
    it('should manually trigger processing via processBatchService', async () => {
      ...
      // Assert
      expect(mockLogger.log).toHaveBeenCalledWith(
        '🔧 Manual notification processing triggered',
      );
      expect(mockProcessBatchService.execute).toHaveBeenCalledTimes(1);
    ...
    it('should stop all registered cron jobs', () => {
      ...
      expect(mockLogger.warn).toHaveBeenCalledWith(
        expect.stringContaining('Stopped job: send-pending-notifications'),
      );
    });
    ...
    it('should start all registered cron jobs', () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        expect.stringContaining('Started job: send-pending-notifications'),
      );
    });
