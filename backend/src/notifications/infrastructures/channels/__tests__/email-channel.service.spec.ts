...
import { EmailChannelService, EmailData } from '../../../infrastructures/channels/email-channel.service';
...
describe('EmailChannelService', () => {
    ...
    beforeAll(async () => {
        ...
        (service as any).delay = jest.fn((ms) => new Promise(resolve => resolve(void 0)));
    });
    ...
    describe('generateReminderEmail', () => {
        ...
        it('should generate email data correctly with all details (populated doctor)', () => {
            ...
            expect(result.html).toContain('📅 <strong>Tanggal:</strong> Kamis, 20 November 2025');
            expect(result.html).toContain('🕐 <strong>Jam:</strong> 17:00 WIB');
            expect(result.html).toContain('👨‍⚕️ <strong>Dokter:</strong> Dr. Gigi Ayu');
            expect(result.html).toContain('📝 <strong>Keluhan:</strong> Sakit gigi berlubang');
        });
        ...
        it('should generate email data correctly when complaint is missing', () => {
            ...
            // Assert
            expect(result.to).toBe(mockNotificationNoComplaint.appointment.patient.email);
        ...
        it('should use doctor_id if doctor object is not populated (string)', () => {
            ...
            // Assert
            expect(result.html).toContain('👨‍⚕️ <strong>Dokter:</strong> doc-string-789');
        });
        ...
        it('should generate a template with the current year in the footer', () => {
            ...
            // Assert
            expect(result.html).toContain(`© ${currentYear} Klinik Dentizy. All rights reserved.`);
        });
    });
});