...
describe('CreatePatientDto', () => {
  ...
  describe('nama_lengkap', () => {
    it('should fail if nama_lengkap is empty', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, nama_lengkap: '' });
      const errors = await validate(dto);
    ...
    it('should fail if nama_lengkap is shorter than 3 chars', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, nama_lengkap: 'Al' });
      const errors = await validate(dto);
    ...
    it('should fail if nama_lengkap is longer than 250 chars', async () => {
      ...
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, nama_lengkap: longName });
      const errors = await validate(dto);
  ...
  describe('nik', () => {
    it('should fail if nik is not 16 digits (length check)', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, nik: '123' });
      const errors = await validate(dto);
    ...
    it('should fail if nik contains non-numeric characters', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, nik: '123456789012345a' });
      const errors = await validate(dto);
  ...
  describe('email', () => {
    it('should fail if email format is invalid', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, email: 'budi-not-email' });
      const errors = await validate(dto);
  ...
  describe('no_hp', () => {
    it('should fail if phone number format is invalid (e.g. too short)', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, no_hp: '081' });
      const errors = await validate(dto);
    ...
    it('should fail if phone number contains letters', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, no_hp: '08123456abc' });
      const errors = await validate(dto);
    ...
    it('should accept valid formats (+62, 62, 0)', async () => {
        ...
        for (const phone of cases) {
            const dto = plainToInstance(CreatePatientDto, { ...validBaseData, no_hp: phone });
            const errors = await validate(dto);
            expect(errors.length).toBe(0);
        }
    });
  ...
  describe('tanggal_lahir', () => {
    it('should fail if date string is invalid', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, tanggal_lahir: '01-01-1990' }); // Wrong format
      const errors = await validate(dto);
  ...
  describe('jenis_kelamin', () => {
    it('should fail if value is not a valid enum member', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, jenis_kelamin: 'ALIEN' as Gender });
      const errors = await validate(dto);
  ...
  describe('optional text fields (limits)', () => {
    it('should validate maxLength for alamat', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, alamat: 'a'.repeat(501) });
      const errors = await validate(dto);
    ...
    it('should validate maxLength for riwayat_alergi', async () => {
      const dto = plainToInstance(CreatePatientDto, { ...validBaseData, riwayat_alergi: 'a'.repeat(1001) });
      const errors = await validate(dto);
