...
const userSchema = z.object({
    ...
    // Password Validation Strict (Sama seperti backend)
    password: z.string().optional().refine((val) => {
        if (!val) return true; 
        return PASSWORD_REGEX.test(val);
    }, { 
        message: "Password harus mengandung Huruf Besar, Kecil, Angka, Simbol & Min 8 Karakter" 
    }),
    // [DIHAPUS] specialization dari schema
});
...
export default function UserFormModal({ isOpen, onClose, initialData, onSuccess }: UserFormModalProps) {
    ...
    const roleOptions = useMemo(() => {
        const list = Array.isArray(rolesData) ? rolesData : (rolesData as any)?.data || [];
    ...
    const form = useForm({
        initialValues: {
            ...
            password: '',
            roleId: '',
            // [DIHAPUS] specialization dari initialValues
        },
        ...
        onSubmit: async (values) => {
            try {
                if (!isEdit && !values.password) {
                    form.setFieldError('password', 'Password wajib diisi untuk pengguna baru');
                    return;
                }
                ...
                const payload: any = {
                    ...
                    // [DIHAPUS] specialization dari payload
                };
    ...
    useEffect(() => {
        if (isOpen) {
            form.resetForm();
            if (initialData) {
                const currentRoleId = initialData.roles?.[0]?.id?.toString() || '';
                form.setValues({
                    ...
                    // [DIHAPUS] specialization dari populate
                });
            }
        }
    ...
    return (
        <Modal
            ...
            description={isEdit ? "Perbarui informasi akun & akses." : "Buat akun untuk staf atau dokter klinik."}
            size="lg"
        >
            <form onSubmit={form.handleSubmit} className="space-y-4 py-4" autoComplete="off">
