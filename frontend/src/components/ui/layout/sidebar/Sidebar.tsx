...
import { NavItem, RoleBadge, SidebarSkeleton, EmptyNavigation, SidebarHeader } from './SidebarComponents';
...
export default function Sidebar() {
    ...
    const getPrimaryRole = () => {
        ...
        const roleNames = currentUser.roles.map((role: string | Role) => (typeof role === 'string' ? role : role.name));
    ...
    return (
        <aside className={`bg-gray-800 text-white transition-all duration-300 ease-in-out flex flex-col h-screen overflow-hidden ${isMinimized ? 'w-16' : 'w-64'}`}>
            <SidebarHeader isMinimized={isMinimized} onToggle={() => setIsMinimized(!isMinimized)} />
                ...
                {loading ? (
                    ...
                    <ul className="space-y-2">
                        {filteredNavigation.map((item, index) => (
                            <NavItem key={item.id} item={item} isActive={isActiveRoute(item.href)} isMinimized={isMinimized} index={index} />
                        ))}
                    </ul>
                )}
            </nav>
            ...
            <footer className="border-t border-gray-700 p-4 flex-shrink-0">
                {loading ? (
                    ...
                    <div className={`flex items-center transition-all ${isMinimized ? 'justify-center' : 'space-x-3'}`}>
                        <Avatar name={currentUser?.nama_lengkap || 'User'} size="md" />
                        {!isMinimized && (
                            <div className="min-w-0 flex-1 animate-fade-in">
                                <p className="text-sm font-medium truncate">{currentUser?.nama_lengkap || 'User'}</p>
                                <div className="mt-1">
